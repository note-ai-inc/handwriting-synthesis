...
# Style caching to avoid repeated file I/O
_style_cache = {}
_style_cache_lock = threading.Lock()

def get_cached_style(style_id):
    """Get style data from cache or load from file if not cached."""
    with _style_cache_lock:
        if style_id not in _style_cache:
            ...
            _style_cache[style_id] = (style_strokes, style_chars)
...
def build_input_tensors(item, text_len):
    ...
    """
    style_id = item["style"]
    style_strokes, style_chars = get_cached_style(style_id)
    text = item["line"]
    ...
    # Optimized sampling steps: reduce from 40x to 20x for better performance
    # This still provides good quality while being much faster
    max_tsteps = 20 * max(1, text_len)
...
def process_batch_items(items, ref_strokes=None, screen_width=800, screen_height=600):
    """
    Process multiple items in a single TensorFlow session run to reduce thread lock overhead.
    This is much more efficient than processing items individually.
    """
    if not items:
        return []
    ...
    max_text_len = max(len(item["line"]) for item in items)
    max_tsteps = 20 * max(1, max_text_len)
    
    # Initialize batch arrays
    x_prime_batch = np.zeros((batch_size, 1200, 3), dtype=np.float32)
    x_prime_len_batch = np.zeros((batch_size,), dtype=np.int32)
    chars_batch = np.zeros((batch_size, 120), dtype=np.int32)
    chars_len_batch = np.zeros((batch_size,), dtype=np.int32)
    bias_batch = np.zeros((batch_size,), dtype=np.float32)
    ...
    for i, item in enumerate(items):
        ...
        chars_batch[i, :len(full_char_seq)] = full_char_seq
        chars_len_batch[i] = len(full_char_seq)
        
        x_prime_batch[i, :len(style_strokes), :] = style_strokes
        x_prime_len_batch[i] = len(style_strokes)
        
        bias_batch[i] = item["bias"]
    
    # Use reference strokes if provided (use for first item only)
    if ref_strokes is not None and len(ref_strokes) > 0:
        try:
            ref_strokes_array = np.array([[s['x'], s['y'], s['eos']] for s in ref_strokes], dtype=np.float32)
            ...
            # Use reference strokes for the entire batch
            x_prime_batch = np.tile(ref_strokes_array.reshape(1, -1, 3), (batch_size, 1, 1))
            x_prime_len_batch = np.full((batch_size,), len(ref_strokes_array), dtype=np.int32)
            logging.info(f"Using normalized reference strokes for batch with shape {x_prime_batch.shape}")
        except Exception as e:
            logging.error(f"Error with reference strokes in batch: {e}")
    
    # Single TensorFlow session run for the entire batch
    with tf_lock:
        [sampled_batch] = style_model.session.run(
            [style_model.sampled_sequence],
            feed_dict={
                style_model.prime: True,
                style_model.x_prime: x_prime_batch,
                style_model.x_prime_len: x_prime_len_batch,
                style_model.ref_x: x_prime_batch,
                style_model.ref_x_len: x_prime_len_batch,
                style_model.num_samples: batch_size,
                style_model.sample_tsteps: max_tsteps,
                style_model.c: chars_batch,
                style_model.c_len: chars_len_batch,
                style_model.bias: bias_batch
            }
        )
    ...
    results = []
    for i, item in enumerate(items):
        # Strip padding rows for this item
        seq = sampled_batch[i][~np.all(sampled_batch[i] == 0.0, axis=1)]
        ...
        logging.info(f"Processed batch item {item['index']}")
